// Paimon Store - Firebase Enabled Version
// ----------------------------------------
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Paimon Store initialized with Firebase');
    setupNavigation();
    loadSettings();
    fetchAccounts(); // from Firebase instead of localStorage
});
...
async function saveAccountToFirebase(accountData, isEdit = false) {
    if (isEdit) {
        await db.collection("accounts").doc(accountData.id).update(accountData);
    } else {
...
function saveAccount() {
    ...
    if (imageFile) {
        ...
        if (currentEditingAccount) {
            ...
            accountData.id = currentEditingAccount.id;
            finalizeAccountSave(accountData, true);
        } else {
            accountData.image = `https://via.placeholder.com/300x200/2DA9E0/FFFFFF?text=${encodeURIComponent(name)}`;
            finalizeAccountSave(accountData, false);
        }
    }
}

function finalizeAccountSave(accountData, isEdit = false) {
    if (currentEditingAccount) {
        accountData.id = currentEditingAccount.id;
        saveAccountToFirebase(accountData, true);
    } else {
        saveAccountToFirebase(accountData, false);
    }
    hideAddAccountForm();
...
async function toggleAccountStatus(accountId) {
    ...
    await saveAccountToFirebase(account, true);
    renderAccounts();
    renderAdminAccounts();
}
...
function deleteAccount(accountId) {
    if (confirm('Are you sure you want to delete this account? This action cannot be undone.')) {
        deleteAccountFromFirebase(accountId);
    }
}
